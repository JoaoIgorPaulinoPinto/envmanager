using envmanager.src.data.service.dtos;
using envmanager.src.data.service.interfaces;
using static envmanager.src.data.service.dtos.InviteDtos;

namespace envmanager.src.services.usecases.invitation
{
    public class ResponseInvitationUseCase : IResponseInvitationUseCase
    {
        private readonly IInviteRepository _inviteRepository;

        public ResponseInvitationUseCase (IInviteRepository inviteRepository)
        {
            _inviteRepository = inviteRepository;
        }
        public Task<ResponseInviteResponse> Execute(ResponseInviteRequest response, string client)
        {
            return _inviteRepository.AnswerInvite(response, client);
        }

    }
}
