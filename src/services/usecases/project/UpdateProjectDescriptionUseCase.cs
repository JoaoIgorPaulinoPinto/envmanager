...
namespace envmanager.src.services.usecases.project
{
    public class UpdateProjectDescriptionUseCase : IUpdateProjectDescriptionUseCase
    {
        ...
        public async Task<bool> Execute(string description, string projectId, string userId)
        {
            if (string.IsNullOrWhiteSpace(description))
                throw new ArgumentException("Project description must be provided and cannot be empty.");
            ...
            return await _projectRepository.UpdateDescription(description, projectId);
        }
    }
}
